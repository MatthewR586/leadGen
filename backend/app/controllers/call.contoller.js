const CallHistory = require("../models/callHistory.model.js");
...
exports.create = (req, res) => {
  ...
  CallHistory.create(CallHistory, (err, data) => {
    if (err)
      res.send({
...
exports.findAll = (req, res) => {
  ...
  CallHistory.getAll(userId, (err, data) => {
    if (err)
      res.send({
...
exports.update = (req, res) => {
  ...
  CallHistory.updateById(
    req.params.id,
    new CallHistory(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.send({
...
exports.delete = (req, res) => {
  ...
  CallHistory.remove(req.query.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.send({
          message: `Not found User with id ${req.params.id}.`,
